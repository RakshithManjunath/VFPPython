...
def generate_muster():
    ...
    for index, row in filtered_holidays_df.iterrows():
        ...
        final_muster_df.loc[final_muster_df['PDATE'].dt.date == holiday_date, 'STATUS'] = hol_type

    # Set 'ATT_STATUS' to 'w/o' for the dates in token_dates, otherwise it will be 'AB'
    for token, dates in token_dates.items():
        final_muster_df.loc[(final_muster_df['TOKEN'] == token) & (final_muster_df['PDATE'].dt.strftime('%Y-%m-%d').isin(dates)), 'STATUS'] = 'WO'
    ...
    for index, row in lvform_df.iterrows():
        ...
        final_muster_df.loc[(final_muster_df['PDATE'].dt.date == lv_start) & (final_muster_df['EMPCODE']==empcode), 'STATUS'] = lv_type
