...
def generate_punch_shift(punches_df, muster_df, g_current_path):
    ...
    # ✅ FIX #1: Move to OUT-date ONLY if (a) cross-day AND (b) IN-date already has a row
    ...
    # ✅ FIX #2: orphan IN/OUT must be AB (NOT MM). MM will come only from muster STATUS == 'MM' later.
    def mark_orphan_out_as_ab(token, comcode, out_dt):
        nonlocal punch_df
        ...
        if exists.empty:
            ...
            # keep status as-is if already computed; else AB
            if pd.isna(punch_df.loc[idx, "PUNCH_STATUS"]) or str(punch_df.loc[idx, "PUNCH_STATUS"]).strip() == "":
                punch_df.loc[idx, "PUNCH_STATUS"] = "AB"
            punch_df.loc[idx, "TOTALTIME"] = ""
            punch_df.loc[idx, "OT"] = ""
            punch_df.loc[idx, "REMARKS"] = ""

    def mark_orphan_in_as_ab(token, comcode, in_dt):
        nonlocal punch_df
    ...
    for _, row in punches_df.iterrows():
        ...
        if mode == 0:
            # if an IN already pending and we get another IN, old IN is orphan -> AB
            if token in pending_in:
                old_in_dt, old_cc = pending_in[token]
                mark_orphan_in_as_ab(token, old_cc, old_in_dt.to_pydatetime())
            pending_in[token] = (dt, comcode)

        else:
            if token not in pending_in:
                mark_orphan_out_as_ab(token, comcode, dt.to_pydatetime())
                continue
    ...
    # Any leftover IN without OUT -> AB
    for token, (in_time, in_cc) in list(pending_in.items()):
        mark_orphan_in_as_ab(token, in_cc, in_time.to_pydatetime())
    pending_in.clear()
    ...
    # -------------------- Save punch.csv (intermediate) --------------------
    punch_df.to_csv(table_paths["punch_csv_path"], index=False)

    # ==================== Generate ppunches.csv (columns preserved) ====================
    ...
    try:
        ...
        rows = []
        max_punches_found = int(pp.groupby(["TOKEN", "PDATE_DAY"]).size().max() or 0)
        max_punches = max(4, max_punches_found)   # ✅ ALWAYS at least 4 slots
        ...
        # ---- BLANK_STATUS after PUNCH_COUNT, then PP_STATUS ----
        if "BLANK_STATUS" not in ppunches.columns:
            ppunches["BLANK_STATUS"] = ""
        else:
            ppunches["BLANK_STATUS"] = ppunches["BLANK_STATUS"].fillna("")
        ...
        for c in ordered_cols:
            if c not in ppunches.columns:
                ppunches[c] = 0 if c == "PUNCH_COUNT" else ""
    ...
    # -----------------------------------------------------------------
    # ✅ IMPORTANT CHANGE: DO NOT force punch.csv MM using ppunches.PP_STATUS
    # MM should come ONLY from muster STATUS == 'MM' (final override below).
    # -----------------------------------------------------------------
    ...
    # Final MM override ONLY from muster STATUS == MM (and MM only in PUNCH_STATUS, not remarks)
    mask_mm = (
...
def generate_punch_flexi(punches_df, muster_df, g_current_path):
    ...
    ghalf_day = int(file_contents[1])
    gfull_day = int(file_contents[2])
    ggrace_time = int(file_contents[3])
    ot_rounding = int(file_contents[4]) if len(file_contents) > 4 else 0
    ...
    # ==============================
    # BUILD punch.csv (normal flexi logic)
    # ==============================

    punch_df = pd.DataFrame(columns=[
        "TOKEN","COMCODE","PDATE",
    ...
    punches_df["TOKEN"] = punches_df["TOKEN"].astype(str).str.strip()
    punches_df["PDTIME"] = pd.to_datetime(punches_df["PDTIME"], errors="coerce")
    punches_df["MODE"] = pd.to_numeric(punches_df["MODE"], errors="coerce").fillna(0).astype(int)
    ...
    for _, row in punches_df.iterrows():
        ...
        if row["MODE"] == 1:
            ...
            # base status from actual minutes
            status = "PR" if minutes >= gfull_day else ("AB" if minutes <= ghalf_day else "HD")
            remarks = ""

            # grace promotion logic like generate_punch_shift
            minutes_with_grace = minutes + ggrace_time

            if status == "AB" and minutes < ghalf_day and minutes_with_grace >= ghalf_day:
                status = "HD"
                remarks = "&"
            elif status == "HD" and minutes < gfull_day and minutes_with_grace >= gfull_day:
                status = "PR"
                remarks = "&"

            overtime = max(0, minutes - gfull_day)

            # OT threshold logic from gsel_date_path last line
            if overtime < ot_rounding:
                overtime = 0

            ot_str = hhmm(overtime)
            ...
            punch_df = pd.concat([
                ...
                pd.DataFrame({
                    ...
                    "PDATE":[pdate_str],
                    "INTIME1":[in_time.strftime("%Y-%m-%d %H:%M")],
                    "OUTTIME1":[dt.strftime("%Y-%m-%d %H:%M")],
                    "INTIME":[in_time.strftime("%Y-%m-%d %H:%M")],
                    "OUTTIME":[dt.strftime("%Y-%m-%d %H:%M")],
                    "TOTALTIME":[hhmm(minutes)],
                    "PUNCH_STATUS":[status],
                    "REMARKS":[remarks],
                    "OT":[ot_str]
    ...
    # -------- fill AB for missing days --------
    for token in muster_df["TOKEN"].astype(str).unique():
        for d in date_range:
            ds = d.strftime("%Y-%m-%d")
            if not ((punch_df["TOKEN"] == token) & (punch_df["PDATE"] == ds)).any():
                punch_df = pd.concat([
                    punch_df,
                    pd.DataFrame({
                        "TOKEN":[token],
    ...
    punch_df = punch_df.sort_values(["TOKEN","PDATE"])
    punch_df.to_csv(table_paths["punch_csv_path"], index=False)

    # ==========================================================
    # NOW BUILD REAL ppunches.csv (PROPER COLUMN STRUCTURE)
    # ==========================================================

    pp = punches_df.copy()
    pp["PDTIME"] = pd.to_datetime(pp["PDTIME"], errors="coerce")
    pp = pp.dropna(subset=["PDTIME"])
    pp["PDATE_DAY"] = pp["PDTIME"].dt.normalize()
    pp = pp.sort_values(["TOKEN","PDATE_DAY","PDTIME"])

    rows = []
    max_punches_found = int(pp.groupby(["TOKEN", "PDATE_DAY"]).size().max() or 0)
    max_punches = max(4, max_punches_found)
    ...
    base = punch_df.copy()
    base["PDATE"] = pd.to_datetime(base["PDATE"]).dt.normalize()
    base["EMPCODE"] = base["TOKEN"]
    base = base[["TOKEN","COMCODE","PDATE","EMPCODE"]].drop_duplicates()

    ppunches = base.merge(punch_wide,on=["TOKEN","PDATE"],how="left")

    ppunches["PUNCH_COUNT"] = ppunches["PUNCH_COUNT"].fillna(0).astype(int)
    ppunches["PP_STATUS"] = ppunches["PP_STATUS"].fillna("")

    for i in range(1,max_punches+1):
        if f"PUNCH_{i}" not in ppunches.columns:
            ppunches[f"PUNCH_{i}"] = ""
        if f"MODE{i}" not in ppunches.columns:
            ppunches[f"MODE{i}"] = pd.Series([pd.NA]*len(ppunches),dtype="Int64")
    ...
    ppunches = ppunches[ordered_cols]
    ppunches["PDATE"] = pd.to_datetime(ppunches["PDATE"]).dt.strftime("%Y-%m-%d 00:00:00")
    ppunches = ppunches.sort_values(["TOKEN","PDATE"])

    ppunches.to_csv(table_paths["ppunches_csv_path"],index=False)
